...
response.setContentType("text/html; charset=UTF-8"); %> <%@ taglib prefix="c"
...
<<<<<<< HEAD
  ...
  <body>
    <% String year = (String) request.getAttribute("year"); String month =
    (String) request.getAttribute("month"); String date = (String)
    request.getAttribute("date"); int member_no = (int)
    ...
    <div id="wrap">
      ...
      <div>
        ...
        <input
          type="text"
          name="calendar_title"
          value="${dto.calendar_title }"
          readonly="readonly"
        />
      </div>
      <div>
        시작 : <input type="text" name="calendar_startdate" value="<%=month %>월
        <%=date %>일
        <c:out value="${fn:substring(dto.calendar_startdate, 8, 10)}:" /><c:out
          value="${fn:substring(dto.calendar_startdate, 10, 12)}"
        />" readonly="readonly"/>
      </div>
      <div>
        종료 : <input type="text" name="calendar_startdate" value="<%=month %>월
        <%=date %>일
        <c:out value="${fn:substring(dto.calendar_enddate, 8, 10)}:" /><c:out
          value="${fn:substring(dto.calendar_enddate, 10, 12)}"
        />" readonly="readonly"/>
      </div>
      <div>
        필수품 :
        <input
          type="text"
          name="calendar_necessity"
          value="${dto.calendar_necessity }"
          readonly="readonly"
        />
      </div>
      <div>
      ...
      <textarea
        ...
        cols="33"
        rows="10"
        placeholder="${dto.calendar_content }"
        readonly="readonly"
      ></textarea>
      <br />
      <input
        type="button"
        value=" 수정 "
        onclick="location.href='../pet.do?command=calendar_calUpdate&calendar_no=${dto.calendar_no }&year=${year }&month=${month }&date=${date }&member_no=${member_no }'"
      />
