	...
	<div id="community_detail_fullbox">
		<div id ="community_detail_contentbox">
			...
			<%if((int)session.getAttribute("member_no")==cdto.getMember_no()){ %>
				...
				<%} %>
			<%} %>
				<button class="filter_button" onclick="location.href='../pet.do?command=community_detail&seq=<%=cdto.getCommunity_seq()%>&community_no=<%=cdto.getCommunity_no()%>&filter=on'">비속어 필터링 OFF </button>
			</div>		
